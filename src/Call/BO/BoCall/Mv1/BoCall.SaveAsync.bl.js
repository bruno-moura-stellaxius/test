...
if(me.getClbStatus() === "Completed" && me.getOriginalClbStatus() !== "Completed"){
  if(Utils.isDefined(me.getBoMyAccountForVisit())){
    me.getBoMyAccountForVisit().setLastVisitDate(new Date());
  }
}
...
if (Utils.isSfBackend()) { // <!-- CW-REQUIRED: Framework is now Utils -->
  ...
  deferreds.push(BoFactory.createObjectAsync("BoSfHelper", {}).then(
    function (helper) {
      ...
      if (me.getLuCallMeta().getCompanyRequired() !== "Not required" || me.getLuCallMeta().getId() == "Joint") {
...
var currentCallPkey = me.getPKey(); 
if (Utils.isDefined(me.getLoMyVisitsForReview())) {
  me.getLoMyVisitsForReview().forEach(function(visit) {
    if(visit.selectedForReview == 1){
      visit.setJointVisit(currentCallPkey);
    }
    
  });
  deferreds.push(me.getLoMyVisitsForReview().saveAsync());
}

if(Utils.isDefined(me.getBoMyAccountForVisit())){
  deferreds.push(me.getBoMyAccountForVisit().saveAsync());
}
