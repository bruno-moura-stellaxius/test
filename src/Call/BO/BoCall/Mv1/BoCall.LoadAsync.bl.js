...
.then(function (boUserDailyReport) {
  ...
  var managerVisitDate = me.dateFrom;
  var jointVisitsQuery = {};
  jointVisitsQuery.params = [
    { "field": "responsiblePKey", "value": me.getMyReviewedUserId() },
    { "field": "addCond", "value": "AND Visit_Template__c.Id != 'Joint' AND "
      +" ((Visit.Status != 'Canceled' AND "
      +" date(date(Visit.PlannedVisitStartTime,'unixepoch', 'localtime')) = '" + managerVisitDate + "')"
      +" OR Visit.JointVisit__c = '"+me.pKey+"')" }
  ];
  return BoFactory.loadListAsync("LoMyJointVisits", jointVisitsQuery);
}).then(function(jointVisits){
  me.setLoMyVisitsForReview(jointVisits);
  return BoFactory.loadObjectAsync("BoMyAccountForVisit", me.getBpaMainPKey());
}).then(function(accountData){
  me.setBoMyAccountForVisit(accountData);
  return me;
});
