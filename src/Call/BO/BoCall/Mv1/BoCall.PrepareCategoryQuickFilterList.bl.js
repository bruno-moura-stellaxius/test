  ...
  promise = BoFactory.createObjectAsync("LoProductQuickFilter", {}).then(
    function (quickFilterList){
      ...
      if(load == "1"){
        ...
        if(Utils.isDefined(loCurrentSurveyProducts)){
          loCurrentSurveyProducts.setCurrentByPKey("");
        }
        me.boJobManager.loMyAccountPromotion.setCurrentByPKey("");
        me.boJobManager.loMyTrainings.setCurrentByPKey("");
      } else{
        var loProductQuickFilter = me.getLoProductQuickFilter();
        surveyItems = loCurrentSurveyProducts.getItems();
        if(surveyItems.length > 0){
