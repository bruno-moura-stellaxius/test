...
if (items.length > 0) {
  ...
  promise = promotionPromise.then(function () {
    ...
    for (var j = 0; j < items.length; j++) {
      ...
      if (items[j].getParentType() === "FreeGoods" || items[j].getParentType() === "FreeOfCharge") {
        ...
        if (itemMeta.getActive() == "1" && itemMeta.getValidFrom() <= Utils.createAnsiDateToday() && itemMeta.getValidThru() >= Utils.createAnsiDateToday() ) {
          // When the Product is a FreeItem and its Order Item Template is valid, reset invalidated flag
