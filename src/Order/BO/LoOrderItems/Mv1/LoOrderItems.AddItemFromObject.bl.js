...
productInfoJsonParams.push({
  "field" : "ordererPKey",
  "value" : Framework.getProcessContext().mainBO.luOrderer.pKey
});
productInfoJsonParams.push({
  "field" : "ordererIndustryCode",
  "value" : Framework.getProcessContext().mainBO.luOrderer.myIndustryCode
});
  ...
  function (lookupData) {
    ...
    if (existingItems.length === 0) {
      ...
      if (Utils.isDefined(product)) {
        ...
        item.myApplyDeliveryUnit = itemTemplate.getMyApplyDeliveryUnit().toString();
        item.myDeliveryUnit = productInformationLookup.getMyDeliveryUnit();
        item.mySAPCode = productInformationLookup.getMySAPCode();
        item.orderItemTemplate = itemTemplate.getOrderItemTemplate();
        item.refPKey = productPKey + itemTemplate.getPKey();
        item.oneHundredDiscount = itemTemplate.getOneHundredDiscount();
        item.deletedFreeItem = "0";
        item.sort = "0";
        item.discount = 100;
        item.specialPrice = 0;
        item.myDeliveryValidThru = productInformationLookup.getMyDeliveryValidThru();
        item.myDeliveryValidFrom = productInformationLookup.getMyDeliveryValidFrom();
        item.mySalesRepQuota = productInformationLookup.getSalesRepQuota();
        item.mySalesRepQuotaCategory = productInformationLookup.getSalesRepQuotaCategory();
        item.mySalesRepQuotaValidFrom = productInformationLookup.getSalesRepQuotaValidFrom();
        item.mySalesRepQuotaValidTo = productInformationLookup.getSalesRepQuotaValidTo();
        item.myChannelQuota = productInformationLookup.getChannelQuota();
        item.myChannelQuotaCategory = productInformationLookup.getChannelQuotaCategory();
        item.myChannelQuotaValidFrom = productInformationLookup.getChannelQuotaValidFrom();
        item.myChannelQuotaValidTo = productInformationLookup.getChannelQuotaValidTo();
        item.myCustomerQuota = productInformationLookup.getCustomerQuota();
        item.myCustomerQuotaCategory = productInformationLookup.getCustomerQuotaCategory();
        item.myCustomerQuotaValidFrom = productInformationLookup.getCustomerQuotaValidFrom();
        item.myCustomerQuotaValidTo = productInformationLookup.getCustomerQuotaValidTo();
        item.pricingInfo1 = 0;
        item.pricingInfo2 = 0;
        item.pricingInfo3 = 0;
        item.pricingInfo4 = 0;
        ...
        while (Utils.isDefined(currentPContext.mainBO['myDDSplit'+splitIndex]) && 
        currentPContext.mainBO['myDDSplit'+splitIndex] != Utils.getMinDate() ) {
          let splitDeliveryDate = new Date(currentPContext.mainBO['myDDSplit'+splitIndex]);
          item["mySplit"+splitIndex+"DD"] = splitDeliveryDate.toLocaleDateString('en-in',{day:'2-digit',month:'2-digit'});
          splitIndex++;
        }
