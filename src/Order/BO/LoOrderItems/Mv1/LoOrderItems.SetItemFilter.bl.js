...
if(Utils.isDefined(promotionPKey) && !Utils.isEmptyString(promotionPKey)){
  ...
  paramsForPkeys = [{"promotionPKey":" ", "op":"EQ"},{"quantity":"0", "op":"GT"},{"showInBasket" : "1", "op" : "EQ"},{"showInFreeGoods" : "1", "op" : "EQ"}];
}
...
switch (filterCode) 
{
  ...
  case "FreeGoods":
    ...
    // do not show deleted free Items
    filterArray.push({"deletedFreeItem" : "0", "op" : "EQ"});
    //show Free Goods relevant items
    filterArray.push({"showInFreeGoods" : "1", "op" : "EQ"});
    break;
}
...
if(filterCode != 'FreeGoods'){
  filterArray.push({"myIsBOMPart" : "0", "op" : "EQ"});
}
var currentPContext = Framework.getProcessContext();
if(currentPContext.mySelectedTab == 'tab_ExpiredItemList' ){
  filterArray.push({"myEligibleForExchange" : "1", "op" : "EQ"});
  filterArray.push({"myPrdState" : {"4": "4", "5": "5"}, "op" : "IN"});
}else if(currentPContext.mySelectedTab == 'tab_ExchangeItemList'){
  filterArray.push({"myAvailableForExchange" : "1", "op" : "EQ"});
  filterArray.push({"myPrdState" : "4", "op" : "EQ"});
}
else if(filterCode == 'All'){
  filterArray.push({"prdType" : {"ZTGO": "ZTGO", "ZTGD": "ZTGD","ZDIS": "ZDIS"}, "op" : "IN"});
}
...
currentPContext.mainBO = currentPContext.mainBO ? currentPContext.mainBO : currentPContext.myMainOrder;

if(currentPContext.mainBO.getBoOrderMeta().id == 'Exchange'){
  currentPContext.mainBO.setMySelectedTab(currentPContext.mySelectedTab);
  currentPContext.mainBO.calculateOrderValue();
}
currentPContext.mainBO.setItemFilterCounts();
