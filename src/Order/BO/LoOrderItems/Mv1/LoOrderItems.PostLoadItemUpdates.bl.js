...
for(var i=0; i < orderItems.length; i++) {
  ...
  //Set the myTotalValue for Exchange Order
  if(orderItem.getMyEligibleForExchange() == '1'){
    if(orderItem.getMyPrdDepreciationPercentage()){
      orderItem.setValue( parseFloat(orderItem.getSimplePricingBasePrice().toString().replace(",",".")) * (1 - parseFloat(orderItem.getMyPrdDepreciationPercentage())/100) * orderItem.getQuantity());
    }else{
      orderItem.setValue( parseFloat(orderItem.getSimplePricingBasePrice().toString().replace(",",".")) * orderItem.getQuantity());
    }
  }else if(orderItem.getMyAvailableForExchange() == '1'){
    orderItem.setValue( parseFloat(orderItem.getSimplePricingBasePrice().toString().replace(",",".")) * orderItem.getQuantity());
  }
}
