...
function myFindExchangeOrder(){
    ...
    var promise = BoFactory.loadListAsync("LoOrderOverview")
    .then(function (boList) {
      if(Utils.isDefined(me.loMyOrderSplits)){
        ...
        BoFactory.loadListAsync("LoMyOrderSplits", { "mainOrderPKey": me.getPKey()})
        .then(LoMyOrderSplits => {
              ...
              if(Utils.isDefined(allSplitOrders)){
                if(allSplitOrders.length > 0 && Utils.isEmptyString(me.myExchangeOrder) && me.myOrderType != 'Exchange'){
                    for(var i = 0; i < allSplitOrders.length; i++){
                      for(var j = 0; j < boList.getAllItems().length; j++){
                        if(!Utils.isEmptyString(boList.getAllItems()[j].mySplitForExchange) && boList.getAllItems()[j].mySplitForExchange == allSplitOrders[i].pKey && boList.getAllItems()[j].myExchangeType == 'Exchange'){
                          me.myExchangeOrder = boList.getAllItems()[j].pKey;
                        }
                      }
                    }
