...
if(isNaN(me.getGrossTotalValueReceipt())){
  me.setGrossTotalValueReceipt(me.getGrossTotalValue());
}
...
var splitsListPromise = when.resolve();


if (orderHasSplits()) {
  splitsListPromise = pricingJsonPromise.then (
    function() {
     return BoFactory.createListAsync(LO_MYORDERSPLITS, {'params': []});
    }
  );
}

/*var orderSplitItemsPromise = splitsListPromise.then(function(loMyOrderSplits) {
  if(Utils.isDefined(loMyOrderSplits)) {
    me.setLoMyOrderSplits(loMyOrderSplits);
    var deliverySplits = [];
    var splitCount = 1;
    while (me['myDDSplit'+splitCount]) {
      deliverySplits.push(me['myDDSplit'+splitCount]);
      splitCount++;
    }
    var deferreds = [];
    for (var i = 0; i < deliverySplits.length; i++) {
      var orderSplitPromise =  BoFactory.createObjectAsync(BO_MYORDERSPLIT, 
        {
          deliveryDate: deliverySplits[i],
          mainOrderPKey: me.pKey,
          loItems: me.loItems,
          splitIndex: i
        });
        deferreds.push(orderSplitPromise);
    }
    return when.all(deferreds);
  }
});
var orderSplitsAddedPromise = orderSplitItemsPromise.then(function(promises) {
  if (Utils.isDefined(promises)) {
    me.loMyOrderSplits.addItems(promises);
  }
}); */
...
var userExitPromise = splitsListPromise.then(
  function () {
