...
// TODO: refactor in  a proper method
function orderHasSplits() {
  return me['myDDSplit1']; // if defined, then we have splits
}
...
if (me.isEditable()) {
  ...
  if (!Utils.isDefined(me.getDeliveryDate()) || Utils.isEmptyString(me.getDeliveryDate())) {
    saveSplitsStack.push(me.setDeliveryDate(Utils.getMinDate())); //the asynchronous eventhandler makes this setter asynchronous
  }
  if(me.getMyIsSplitOrder() == false && me.getBoOrderMeta().getId() != 'Exchange' && (!Utils.isDefined(me.cancelingSplits) || !me.cancelingSplits)) {
    saveSplitsStack.push(me.mySaveSplits());
  }

  promise = when.all(saveSplitsStack)
    .then(function (){
    ...
    if(me.getBoOrderMeta().getId() == "Exchange"){
      var expiredProducts = me.getLoItems().getAllItems().filter((oi) => oi.myEligibleForExchange == "1");
      var exchangeProducts = me.getLoItems().getAllItems().filter((oi) => oi.myAvailableForExchange == "1");
      me.getLoItems().removeAllItems();
      me.getLoItems().addItems(expiredProducts);
      deferreds.push(me.mySaveSplitForExchange(exchangeProducts));
    }
    if (me.getLoItems() && me.getLoItems().getAllItemsCount() > 0) {
    ...
    */
    ...
    // TODO: Check this cover list of lists?? Order splits probably won't be covered
