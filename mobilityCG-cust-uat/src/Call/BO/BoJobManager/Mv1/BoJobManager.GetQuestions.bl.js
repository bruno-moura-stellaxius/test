...
if (Utils.isDefined(liCurrentPOS) && liCurrentPOS.getQuestionsInitialized()!=="1" && (me.getClbStatus() == "Planned" || ignoreCallState)) {
  //var jobDefs = me.getLoJobDefinitions().getItemsByParam({"jobMetaId":"Question", "pOS": liCurrentPOS.getIsPOS(), "myIsFeedbackSurvey": 1});
  var params = {
    "jobMetaId": "Question",
    "pOS": liCurrentPOS.getIsPOS()
  };
  
  // Conditionally add 'myIsFeedbackSurvey' parameter if the template name is joint
  /*if (me.getLuCallMeta().getId() == 'Joint') {
    params.myIsFeedbackSurvey = 1;
  }*/
  var jobDefs = me.getLoJobDefinitions().getItemsByParam(params);
    ...
    if ( (isStorePOS || matchingPOS) || (liCurrentPOS.getIsPOS() === "0" && Utils.isDefined(existingJobsOfThisCall) ) ) {
      if (createQuestion) {
        var questionitem = {
          ...
          "mySurveyTypeId" : questionJobDef.getMySurveyTypeId()
        };
  ...
  if(me.surveyType == "StoreCheck"){
    loQuestions.resetAllFilters();
    loQuestions.setFilterArray([ {"done" : "0", "op" : "EQ"},  {"mySurveyTypeId" : "Store Check", "op" : "EQ"} ]);
  }
  if(me.surveyType == "Feedback"){
    loQuestions.resetAllFilters();
    loQuestions.setFilterArray([ {"done" : "0", "op" : "EQ"},  {"mySurveyTypeId" : "Feedback", "op" : "EQ"} ]);
  }
  if(me.surveyType == "Satisfaction"){
    loQuestions.resetAllFilters();
    loQuestions.setFilterArray([ {"done" : "0", "op" : "EQ"},  {"mySurveyTypeId" : "CSAT", "op" : "EQ"} ]);
  }
  /**
