...
if (Utils.isSfBackend()) {
  ...
  jsonQuery.cond = " AND issuePhase IN (#released#, #initial#, #upperInitial#) AND Task.ActivityDate <> #minimumDate# AND Task.ActivityDate <= #cardDate# AND Task.OwnerId = '#userPKey#'";
  jsonQuery.params.push({"field" : "released", "value" : 'Not Started'});
  jsonQuery.params.push({"field" : "initial", "value" : 'In Progress'});
  jsonQuery.params.push({"field" : "upperInitial", "value" : 'Initial'}); // Current definition is uppercase
  ...
  jsonQuery.cond = " AND SvcTodo.IssuePhase IN (#released#, #initial#, #upperInitial#) AND DATETIME(SvcTodo.DueDate) <> #minimumDate# AND DATETIME(SvcTodo.DueDate) <= #cardDate# ";
