...
var myTotalQuantity = mainItem.getMyTotalQuantity();
var price;
...
if(isNaN(basePricePerCurrentUnit)){
  basePricePerCurrentUnit = basePrice;
}
...
else if (discount > 0 && boOrderMeta.id != 'Exchange') {
  price = basePricePerCurrentUnit - (discount * basePricePerCurrentUnit / 100);
}
else {
  price = basePricePerCurrentUnit;
}
if(mainItem.getMyEligibleForExchange() == '1'){
  if(mainItem.getMyPrdDepreciationPercentage()){
    price = price * (1 - parseFloat(mainItem.getMyPrdDepreciationPercentage())/100);
  }
}
...
if(boOrderMeta.id == "Exchange"){
  ...
  if (Utils.isDefined(myTotalQuantity)) {
    prices.grossValue = myTotalQuantity * basePricePerCurrentUnit;
    prices.grossValueReceipt = myTotalQuantity * basePricePerCurrentUnit;
    prices.value = myTotalQuantity * price;
    prices.valueReceipt = myTotalQuantity * price;
  }
}
