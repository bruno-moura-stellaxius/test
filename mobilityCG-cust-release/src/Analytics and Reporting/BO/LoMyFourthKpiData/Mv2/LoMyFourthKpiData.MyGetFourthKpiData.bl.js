 ...
 * @function myGetFourthKpiData
 * @this LoMyFourthKpiData
 * @kind listobject
 * @async
 * @namespace CUSTOM
 * @param {DomPKey} user
 ...
 * @returns promise
 */
function myGetFourthKpiData(user, currentDate){
    var me = this;
    ///////////////////////////////////////////////////////////////////////////////////////////////
    //                                                                                           //
   ...
   "operator" : "EQ",
    ...
    var promise = Facade.getListAsync("LoMyFourthKpiData", jsonQuery)
    .then(function(data) {
        me.removeAllItems();
        me.addItems(data);
        if(salesOrg == "C172"){ //C172 - France - Average Visit per day (Cycle)
            var periodStartDate =  Utils.convertAnsiDate2Date(data[0].periodStartDate);
            var second=1000, minute=second*60, hour=minute*60, day=hour*24;
            var numberOfDays = Math.round((yesterday - periodStartDate) / day);
            var avgDays = (numberOfDays / 7) * 5; 
            var visitCount = data[0] ? data[0].value : 0;
            var avgVisitsPerDay = (!isNaN(avgDays) && avgDays > 0) ? Math.round(visitCount / avgDays) : 0;
            return avgVisitsPerDay; 
        }
