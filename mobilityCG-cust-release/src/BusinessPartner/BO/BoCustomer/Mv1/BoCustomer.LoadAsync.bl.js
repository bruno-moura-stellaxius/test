...
var promise = Facade.getObjectAsync(BO_CUSTOMER, jsonQuery).then(
  function (selfJson) {
    if (Utils.isDefined(selfJson)) {
      ...
      if (me.bankAccountNumber.length > 4) {
        me.bankAccountNumber = 'XXXXXXXXXX'+me.bankAccountNumber.substr(me.bankAccountNumber.length - 4);
      }
  ...
  function (boBpaMeta) {
    ...
    return BoFactory.loadListAsync("LoMyOrders", jsonQuery);
  }).then(
    function (loMyLastOrder) {
      me.setLoMyLastOrder(loMyLastOrder);
      if (loMyLastOrder!=null && loMyLastOrder.getAllItems().length > 0) {
        me.myLastOrderDate = loMyLastOrder.getAllItems()[0].myCreatedDate;
      }
  ...
  function(loCustomerSalesRevenue){
    ...
    // Adapt performance icon
    if (me?.myVisitPerformance?.toLowerCase()?.indexOf('green') >= 0) {
      me.myVisitPerformance = 'Approve24';
    } else if (me?.myVisitPerformance?.toLowerCase()?.indexOf('red') >= 0) {
      me.myVisitPerformance = 'Reject24';
    }
    //set EARights
    me.setEARights();
    //Reset dirty flag
