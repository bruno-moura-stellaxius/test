...
if(!orderItemsQuery.useMergeEngine) {
  ...
  if (Utils.isSfBackend()) {
    ...
    if(me.boOrderMeta.originalText=='Exchange'){
      addCond_ProductState = " ( Product2.State__c='4' OR Product2.State__c='8' OR Product2.State__c='5') ";
    }
    ...
    addCond_ExchangeProduct = " AND (Product2.AvailableAsExchangeProduct__c = '1' OR Product2.EligibleForExchange__c = '1') ";
    if(me.boOrderMeta.originalText=='Exchange'){
      orderItemsParams.push({ "field": "addCond_ExchangeProduct", "value": addCond_ExchangeProduct });
    }
  }
  else {
    if ((typeof allowForeignProducts != "undefined") && (allowForeignProducts != 1)) {
  ...
  var mySoldToPKey = me.luMySoldTo && me.luMySoldTo.myPartnerPKey ? me.luMySoldTo.myPartnerPKey : ' ';
  ...
  jsonQueryProduct.params.push({"field" : "ordererPKey", "value" : customerPKey});
  jsonQueryProduct.params.push({"field" : "soldtoPKey", "value" : mySoldToPKey});

  if(me.boOrderMeta.originalText=='Exchange'){
    jsonQueryProduct.params.push({"field" : "addCond_ExchangeProduct", "value" : addCond_ExchangeProduct});
  }
  ...
  var myjsonQueryQuotas = {};
  myjsonQueryQuotas.params = [];
  myjsonQueryQuotas.params.push({"field" : "ordererPKey", "value" : customerPKey});
  myjsonQueryQuotas.params.push({"field" : "userPKey", "value" : ApplicationContext.get('user').getPKey()});
  myjsonQueryQuotas.params.push({"field" : "orderDate", "value" : me.initiationDate});
  if (Utils.isDefined(me.myDDSplit1)) {
    myjsonQueryQuotas.params.push({"field" : "myDDSplit1", "value" : me.myDDSplit1});
  }
  if (Utils.isDefined(me.myDDSplit2)) {
    myjsonQueryQuotas.params.push({"field" : "myDDSplit2", "value" : me.myDDSplit2});
  }
  if (Utils.isDefined(me.myDDSplit3)) {
    myjsonQueryQuotas.params.push({"field" : "myDDSplit3", "value" : me.myDDSplit3});
  }
  if (Utils.isDefined(me.myDDSplit4)) {
    myjsonQueryQuotas.params.push({"field" : "myDDSplit4", "value" : me.myDDSplit4});
  }
  if (Utils.isDefined(me.myDDSplit5)) {
    myjsonQueryQuotas.params.push({"field" : "myDDSplit5", "value" : me.myDDSplit5});
  }
  if (Utils.isDefined(me.myDDSplit6)) {
    myjsonQueryQuotas.params.push({"field" : "myDDSplit6", "value" : me.myDDSplit6});
  }
  myjsonQueryQuotas.params.push({"field" : "orderType", "value" : me.boOrderMeta.getId()});

  var myJsonQueryTargetProducts = {};
  myJsonQueryTargetProducts.params = [];
  myJsonQueryTargetProducts.params.push({"field" : "customerPKey", "value" : customerPKey});

  var myJsonQueryProductBasePrice = {};
  myJsonQueryProductBasePrice.params = [];
  myJsonQueryProductBasePrice.params.push({"field" : "ordererPKey", "value" : customerPKey});
  myJsonQueryProductBasePrice.params.push({"field" : "orderDate", "value" : me.initiationDate});

  var myJsonQueryCycleBrands = {};
  myJsonQueryCycleBrands.params = [];
  myJsonQueryCycleBrands.params.push({"field" : "orderDate", "value" : me.initiationDate});

  // create sql statements
  var queryParams = [];
  var loadProductQuery = Facade.myProxy.getLoadStatement("LoMeProductInformation", jsonQueryProduct);
  if(loadProductQuery.sql.includes('case when Product2.State__c = \'4\' then \'4\' when Product2.State__c = \'8\' then \'8\' else \'4\' end as myPrdState')){
    loadProductQuery.sql = loadProductQuery.sql.replace('case when Product2.State__c = \'4\' then \'4\' when Product2.State__c = \'8\' then \'8\' else \'4\' end as myPrdState','Product2.State__c as myPrdState')
  }
  ...
  var myQuotasForProducts = Facade.myProxy.getLoadStatement("LoMyProductQuotas", myjsonQueryQuotas);

  var myTargetProducts = Facade.myProxy.getLoadStatement("LoMyTargetProducts", myJsonQueryTargetProducts);

  var myProductBasePrice = Facade.myProxy.getLoadStatement("LoMyProductBasePrice", myJsonQueryProductBasePrice);

  var myCycleBrands = Facade.myProxy.getLoadStatement("LoMyCycleTopBrands", myJsonQueryCycleBrands);
  ...
  query += "LEFT OUTER JOIN (" + myQuotasForProducts.sql + ") AS myProductQuotas ON myProductQuotas.quotaProductId = product.pKey ";
  queryParams = queryParams.concat(myQuotasForProducts.params);

  query += "LEFT OUTER JOIN (" + myTargetProducts.sql + ") AS myTargetProducts ON myTargetProducts.productId = product.pKey ";
  queryParams = queryParams.concat(myTargetProducts.params);

  query += "LEFT OUTER JOIN (" + myProductBasePrice.sql + ") AS myProductBasePrice ON myProductBasePrice.productId = product.pKey ";
  queryParams = queryParams.concat(myProductBasePrice.params);

  query += "LEFT OUTER JOIN (" + myCycleBrands.sql + ") AS myCycles ON myCycles.Brand = product.criterion3 ";
  queryParams = queryParams.concat(myCycleBrands.params);
