...
var promise = Facade.getObjectAsync(BO_ORDERMETA, context.jsonQuery).then(
  function (selfJson) {
    ...
    idToUse = me.getPKey();
    return BoFactory.loadListAsync(LO_ORDERITEMMETAS, Utils.getParamsForDeprecatedDS("sdoMetaPKey", idToUse));
  ...
  function (loOrderItemMetasJson) {
    if (Utils.isDefined(loOrderItemMetasJson.getMainItemTemplate()) ||
      ...
      return BoFactory.loadListAsync(LU_ORDERMETAFORFREEITEMS, me.getQueryBy("sdoMetaPKey", idToUse)).then(
        function (luOrderMetaForFreeItems) {
          me.setLuOrderMetaForFreeItems(luOrderMetaForFreeItems);

          return BoFactory.loadListAsync(LO_INVENTORYMETABYITEMMETA, me.getQueryBy("sdoMetaPKey", idToUse));
        }).then(
        ...
        function (recordType) {
          ...
          return BoFactory.loadListAsync("LoInventoryMetaByPaymentMeta", me.getQueryBy("sdoMetaPKey", idToUse));
        }).then(
        ...
        function (loPrdGroupLevelCriterionMappingJson) {
          ...
          if ((me.getItemListOption() == "Hierarchy") && (me.getMobilityRelevant() == "1")) {
            return BoFactory.loadListAsync(LO_ORDERMETAPRDSELECTOR, me.getQueryBy("sdoMetaPKey", idToUse));
          } else {
            return undefined;
          }
        }).then(
