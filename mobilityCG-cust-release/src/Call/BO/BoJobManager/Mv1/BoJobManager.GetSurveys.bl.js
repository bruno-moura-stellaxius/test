 ...
 * @param {String} mySurveyType
 * @returns promise
 */
function getSurveys(posIdParam, prdMainPKeyParam, jobProductPKeyParam, mySurveyType){
    var me = this;
    ///////////////////////////////////////////////////////////////////////////////////////////////
    //                                                                                           //
...
if (loCurrentPOS.length > 0) {
  ...
  if (Utils.isDefined(loSurveys)) {
    if (Utils.isDefined(prdMainPKeyParam) && !Utils.isEmptyString(prdMainPKeyParam)) {
      ...
      if(mySurveyType == "Promotions" || mySurveyType == "Trainings"){
        var loCurrentProduct = loProducts.getItemsByParam({"prdMainPKey": jobProductPKeyParam});
      }
      ...
      if (loCurrentProduct.length > 0) {
        ...
        if (surveysNotInitialized && me.getClbStatus() === "Planned") {
              ...
              for (idxJobDef = 0; idxJobDef < jobDefs.length; idxJobDef++) {
                ...
                if (bCreateSurvey || (!wasAnsweredInThisCall && liJobDefinition.presetting !== "LastValue")) {
                  newLiSurvey = {
                    ...
                    "mySurveyTypeId":liJobDefinition.getMySurveyTypeId(),
                    "myPromotionId":liJobDefinition.getMyPromotionId(),
                    "myTrainingCommitmentId":liJobDefinition.getMyTrainingCommitmentId(),
                    "myCameraIcon" : (liJobDefinition.getPictureTaking() == "1")  ? "CapturePictureIcon" : "EmptyImage"
                  };
          ...
          if(mySurveyType == "Distribution"){
            loSurveys.resetAllFilters();
            loSurveys.setFilterArray([ {"done" : "0", "op" : "EQ"}, {"objectStatus" : STATE.PERSISTED, "op" : "EQ"}, {"mySurveyTypeId" : "Distribution", "op" : "EQ"} ]);

          } else if(mySurveyType == "Visibility"){
            loSurveys.resetAllFilters();
            loSurveys.setFilterArray([ {"done" : "0", "op" : "EQ"}, {"objectStatus" : STATE.PERSISTED, "op" : "EQ"}, {"mySurveyTypeId" : "Visibility", "op" : "EQ"} ]);
          }
