    ...
    if (Utils.isDefined(loSurveys)) {
      ...
      loSurveys.forEach(function(survey){
        if((survey.getObjectStatus() === 5 || survey.getObjectStatus() === 13) && survey.getDone() == "0"){
          ...
          survey.setObjectStatus(this.self.STATE_NEW_DIRTY);
        }
        
        if(!Utils.isEmptyString(survey.myPromotionId) && survey.getDone() == "1"){
          var accountPromo = me.getLoMyAccountPromotion().getAllItems().filter(function(item){ 
            return survey.done == "1" && item.promotionId == survey.myPromotionId;
          });
          if(Utils.isDefined(accountPromo) && Utils.isDefined(accountPromo[0])){
            accountPromo[0].setLastSalesRepAudit(Utils.createDateToday());
          }
        }
      });
      promises.push(Facade.saveListAsync(loSurveys));
      promises.push(Facade.saveListAsync(me.getLoMyAccountPromotion()));
    }
  }
  if (Utils.isDefined(me.getLoCurrentSurveys())) {
...
if(Utils.isDefined(me.getLoCurrentSurveyProducts())) {
  me.getLoCurrentSurveyProducts().resetAllFilters();
}
