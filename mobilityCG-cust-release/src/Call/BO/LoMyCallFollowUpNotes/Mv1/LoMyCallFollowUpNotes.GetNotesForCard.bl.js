...
var emptyCurrentNoteExists = false;
var counter = 0;
var notesList = [];
...
for(i=0;i<notes.length;i++){
  if(notes[i].pKey === clbMainPKey ){
    if( Utils.isEmptyString(notes[i].text)){
      emptyCurrentNoteExists = true;
    }
    var liCallnote = {
      "pKey": notes[i].pKey,
...
if(emptyCurrentNoteExists) {
  me.cardItemCount = notesList.length-1;
  numberOfListItems = (Number(numberOfListItems) || 0) + 1;
}
else {
...
for(i=0;i<notesList.length;i++) {
  if(!Utils.isPhone()) {
    if(notesList[i].shortText.length > 100) {
      notesList[i].shortText= notesList[i].shortText.substr(0, 100);
